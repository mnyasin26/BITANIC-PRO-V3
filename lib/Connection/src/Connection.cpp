...
{
  EEPROM.begin(4000);
  this->saved_wifi_ssid = EEPROM.readString(100);
  EEPROM.end();
...
{
  EEPROM.begin(4000);
  this->saved_wifi_password = EEPROM.readString(150);
  EEPROM.end();
...
void Connection::mqttReconnect()
{
  ...
  if (!this->client.connected() && WiFi.status() == WL_CONNECTED)
  {
    ...
    this->client.setBufferSize(6000);
...
void Connection::connectMQTT(String server, int port, String id, String user, String password)
{
  ...
  this->client.setCallback(this->mqttCallback);
  this->client.setBufferSize(7000);
  this->mqtt_connected = true;
  if (!this->client.connect(this->id_mqtt.c_str()))
  {
#ifdef DEBUG_MQTT
...
void Connection::startWiFiManagerNonBlocking(String APName, String APPassword, int timeout)
{
  // is configuration portal requested?
  if ((!portalRunning))
  {
    wm.setHostname("BITANIC");
...
// if you get here you have connected to the WiFi
    ...
    EEPROM.begin(4000);
    EEPROM.writeString(100, wm.getWiFiSSID());
    EEPROM.writeString(150, wm.getWiFiPass());
    EEPROM.commit();
    EEPROM.end();
    this->wifi_connected = true;
  }
